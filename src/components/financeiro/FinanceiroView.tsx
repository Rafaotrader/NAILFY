...
import { mockTransactions, mockAppointments } from "@/data/mockData";
...
export default function FinanceiroView() {
  ...
  const monthRevenue = monthTx.filter(t => t.type === "Entrada").reduce((s, t) => s + t.value, 0);
  const monthExpenses = monthTx.filter(t => t.type === "Despesa").reduce((s, t) => s + t.value, 0);
  const pendingPayments = mockAppointments.filter(a => a.paymentStatus === "Pendente" && a.status === "Concluído").reduce((s, a) => s + a.value, 0);
  const signals = monthTx.filter(t => t.type === "Sinal").reduce((s, t) => s + t.value, 0);
  const todayRevenue = mockAppointments.filter(a => a.date === today && a.status === "Concluído").reduce((s, a) => s + a.value, 0);
  ...
  return (
    <div className="px-4 pt-6 pb-4">
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-center">
          <p className="text-[#e91e8c] font-bold">R$105</p>
          <p className="text-zinc-500 text-xs">Ticket medio</p>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-center">
          <p className="text-[#e91e8c] font-bold">Gel</p>
